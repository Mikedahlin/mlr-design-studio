...
const treatments = [
  ...
  },
  {
    ...
    items: ["Childrenâ€™s visits", "Sealants", "Healthy habit coaching"],
  },
];
    ...
    <div className={styles.site}>
      ...
      <main id="main">
        <section className={styles.hero} id="top">
          <Image className={styles.heroImage} src="/media/card-final-frames/white-pine-dental.jpg" alt="A serene White Pine Dental practice interior" fill priority sizes="100vw" />
          <div className={styles.heroShade} />
          <div className={styles.heroContent}>
            <p className={styles.eyebrow}>North woods calm Â· modern dental care</p>
            ...
            <div className={styles.heroActions}>
              <a className={styles.primary} href="#visit">Iâ€™m a new patient</a>
        ...
        <section className={styles.care} id="care" aria-labelledby="care-heading">
          ...
          <div className={styles.explorer}>
            <div className={styles.tabs} role="tablist" aria-label="Treatment categories">
              {treatments.map((item) => <button key={item.id} role="tab" aria-selected={treatment.id === item.id} aria-controls="treatment-panel" id={`tab-${item.id}`} onClick={() => setTreatment(item)}>{item.label}<span aria-hidden="true">â†’</span></button>)}
            </div>
            <div className={styles.treatmentPanel} role="tabpanel" id="treatment-panel" aria-labelledby={`tab-${treatment.id}`}>
              <span className={styles.pineIcon} aria-hidden="true">â™§</span><p className={styles.kicker}>{treatment.label}</p><h3>{treatment.title}</h3><p>{treatment.text}</p><ul>{treatment.items.map((item) => <li key={item}>{item}</li>)}</ul><a href="#request">Ask about this care <span aria-hidden="true">â†’</span></a>
            </div>
          </div>
        </section>

        <section className={styles.visit} id="visit" aria-labelledby="visit-heading">
          <div className={styles.visitMedia}><Image src="/media/white-pine/white-pine-consultation.png" alt="A calm consultation corner with pale pine furnishings" fill sizes="(max-width: 850px) 100vw, 42vw" /></div>
          <div className={styles.visitCopy}><p className={styles.kicker}>Your first visit</p><h2 id="visit-heading">Come as you are. Weâ€™ll take it from there.</h2><p>A first appointment is for understanding your health, priorities, and comfort preferences. Treatment is never assumed.</p><div className={styles.callout}><strong>Prefer a quieter visit?</strong><span>Tell us when you request an appointment. We can note pacing, breaks, and communication preferences.</span></div></div>
        ...
        <section className={styles.request} id="request" aria-labelledby="request-heading">
          <div className={styles.requestIntro}><p className={styles.kicker}>Appointment request</p><h2 id="request-heading">Letâ€™s find a good time.</h2><p>This three-step form demonstrates a possible booking experience.</p><div className={styles.demoBadge}>Concept demo Â· Not connected to a dental practice</div></div>
          <div className={styles.formCard}>
            ...
            {submitted ? <div className={styles.confirm} role="status"><span aria-hidden="true">âœ“</span><h3>Demo complete</h3><p>No appointment was sent and no information was stored. In a live experience, the practice would follow up to confirm availability.</p><button type="button" onClick={() => { setSubmitted(false); setStep(1); }}>Start over</button></div> :
            <form onSubmit={submitDemo}>
              {step === 1 && <fieldset><legend>1. What can we help with?</legend><p>Select the closest match. A real care team would confirm the right visit type.</p><div className={styles.choiceGrid}>{["New patient exam", "Cleaning & checkup", "Specific concern", "Cosmetic consultation"].map(value => <label key={value}><input required type="radio" name="service" value={value} checked={service === value} onChange={() => setService(value)} /><span>{value}</span></label>)}</div><button className={styles.formNext} type="button" disabled={!service} onClick={() => setStep(2)}>Continue to timing â†’</button></fieldset>}
              {step === 2 && <fieldset><legend>2. When works best?</legend><p>Choose a general preference; this demo does not show live availability.</p><div className={styles.choiceGrid}>{["Weekday mornings", "Weekday afternoons", "As soon as available", "Iâ€™m flexible"].map(value => <label key={value}><input required type="radio" name="timing" value={value} checked={timing === value} onChange={() => setTiming(value)} /><span>{value}</span></label>)}</div><div className={styles.formButtons}><button type="button" onClick={() => setStep(1)}>â† Back</button><button className={styles.formNext} type="button" disabled={!timing} onClick={() => setStep(3)}>Continue to details â†’</button></div></fieldset>}
              {step === 3 && <fieldset><legend>3. How could we reach you?</legend><p>For this concept demo, use placeholder information only. Nothing is submitted or stored.</p><div className={styles.fields}><label>First name<input required name="name" autoComplete="given-name" placeholder="Sample name" /></label><label>Email<input required type="email" name="email" autoComplete="email" placeholder="sample@example.com" /></label><label>Phone <span>(optional)</span><input type="tel" name="phone" autoComplete="tel" placeholder="(555) 555-0123" /></label></div><label className={styles.consent}><input required type="checkbox" /> I understand this is a non-functional concept demo.</label><div className={styles.formButtons}><button type="button" onClick={() => setStep(2)}>â† Back</button><button className={styles.formNext} type="submit">Complete demo</button></div></fieldset>}
            </form>}
          </div>
        </section>
